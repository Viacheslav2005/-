    ...
    <form action="" method="GET">
        <select name="cat_id" id="">
            ...
            <?php foreach($cat as $item) { ?>
                <option value="<?=$item[0]?>"><a href="?page=<?=$i?>&search=<?=$ins_name?>&cat_id=<?=$item[0]?>"><?=$item[1]?></a></option>
            <?php } ?>
        </select>
...
<div class="div_programs">
    ...
    <?php if($ins_name)  { ?> 
        <?php foreach($search as $item) { ?>
            <div class="div_programm">
                ...
                <button type="button" class="head_but" onclick="document.location='admin_insurance_polis_up.php?id=<?=$item[0]?>'">Изменить</button>
            </div>
        <?php } ?>
    ...
    <div>
        ...
        <button onclick="document.location = 'admin_insurance_polis.php'">Очистить форму поиска</button>
    </div>
